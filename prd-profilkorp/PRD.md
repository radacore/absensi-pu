...
*   **Karyawan BBWS PJ:** ASN/non-ASN di pusat maupun cabang, login via email + password di mobile PWA untuk absensi GPS+selfie (cek jarak ke **titik proyek terdekat di wilayahnya**, mis. Bendungan A/Jembatan B), cuti berjenjang, pengumuman, profil, **4 Love/bulan (reset 1st 00:00 WITA, fleksibel total love diatur Super Admin, pakai Love untuk excuse terlambat dalam radius dengan dokumen).**
...
|:---|:---|:---|
| **Super Admin Pusat (Makassar)** | Admin pusat di Makassar. | - Full CRUD on all cabang (Kabupaten/Kota se-Sulsel) + semua data karyawan. - Input/edit **N titik proyek per wilayah** (contoh Bendungan A, Jembatan B — masing-masing lat/lng/radius_m via map picker + Leaflet). - Manage akun Admin Wilayah. - Broadcast pengumuman pusat. - Full content, settings, analytics. |
| **Admin Wilayah / Wilayah (Kab/Kota)** | Admin per Kantor Wilayah BBWS PJ di Kabupaten/Kota se-Sulsel. Scope = region_id. | - CRUD data karyawan Lengkap HR **only for own cabang** (read cabang lain, write own). - **Input/edit N titik proyek di wilayahnya sendiri** (Bendungan A, Jembatan B, dst — masing-masing lat, lng, radius_m 50–1000m) untuk geofence absensi karyawannya. - Approve cuti level wilayah. - Kirim pengumuman wilayah. - View attendance own cabang. |
| **Karyawan** (Employee) | ASN/non-ASN pusat/cabang, login email+password, mobile PWA. Strict own-data only. | - View/edit own profile (limited). - Absensi GPS+selfie — divalidasi jarak ke **titik proyek terdekat di wilayahnya** (tiap wilayah bisa punya N titik proyek, contoh Bendungan A/Jembatan B, masing-masing radius fleksibel, di luar semua radius ditolak, tidak tercatat). - Ajukan cuti berjenjang. - **4 Love/bulan (reset bulanan, total fleksibel Super Admin): jika terlambat & masih dalam radius salah satu titik proyeknya di bulan yang sama (hari/tanggal boleh beda), bisa ajukan dokumen/alasan pakai 1 Love untuk excuse late → approval 1 level Admin Wilayah.** - View pengumuman (pusat + wilayahnya). |
...
*   **FR-25: Absensi GPS + Selfie (Mobile PWA, Geofence Titik Proyek):** Karyawan check-in/out via PWA dengan validasi GPS terhadap **titik proyek terdekat di wilayahnya** (N titik per wilayah, contoh Bendungan A, Jembatan B — masing-masing lat/lng + radius_m yang di-input admin). Radius configurable per titik (50–1000m). Selfie upload S3. Records: timestamp, lat/lng, selfie_url, status on-time/late + jarak_m ke titik terdekat (tidak ada out_of_range — di luar semua titik ditolak). Admin Wilayah views attendance cabang sendiri; Super Admin views all cabang. Hitung jarak via Haversine ke tiap titik, pakai `min(distance) <= radius_m(titik_terdekat)`.
...
*   **FR-28: Region/Wilayah Management — N Titik Proyek per Wilayah:** Super Admin CRUD Kantor (Kantor Pusat + Wilayah Kab/Kota se-Sulsel) dengan **N titik proyek per wilayah**, tiap titik punya **nama titik (ex: Bendungan A, Jembatan B), lat/lng via map picker Leaflet, radius absen meter (50–1000, default 200)**. Admin Wilayah dapat **tambah/edit/hapus titik proyek di wilayahnya sendiri** (own region, N titik) dan view cabang lain read-only. Tiap titik punya geofence untuk validasi absensi karyawannya (karyawan lulus jika `distance <= radius_m` ke **salah satu** titiknya). Validasi radius 50–1000m per titik.
*   **FR-29: PWA Mobile Experience:** Employee PWA is installable (manifest + service worker), offline-capable for viewing cached profile/pengumuman, responsive 320px+, touch 44px, push notification ready. Tampilkan jarak ke **titik proyek terdekat** & status dalam radius saat absen (label titik: Bendungan A / Jembatan B).
...
*   **FR-31: Love System — 4 Hati / Bulan, Fleksibel, Dalam Radius Titik Proyek, 1 Level Admin Wilayah (Sebulan):** Setiap karyawan punya **4 Love/bulan** (default, reset `1st 00:00 WITA` tiap bulan, `love_sisa = love_max`). **Total Love fleksibel**: `global_settings.love_max_default` (1–10, default 4) diatur Super Admin Pusat (berlaku bulan depan, log). **Aturan pakai Love**: Jika `absensi.status=late` **dan** `dalam radius salah satu titik proyek di wilayahnya` (Bendungan A / Jembatan B / dst) maka karyawan bisa **ajukan 1 Love Claim di bulan yang sama** dengan late tersebut (hari/tanggal boleh beda, selama masih di bulan kalender yang sama, 1 Love per 1 late) dengan dokumen/alasan via PWA. **Approval 1 level: Admin Wilayah** (own region) approve/reject; approve → `love_sisa-1`, `attendances.status` → `excused_love` (dianggap on_time di rekap), reject → tetap `late`. Di luar semua titik **tidak bisa pakai Love** (absen ditolak 422). Jika `love_sisa=0` → late tidak bisa di-excuse. Tampilan PWA: 4 dot gold `#FCB833`, sisa `3/4`, tombol `Gunakan Love`, history. **Login semua pakai email; Reset password Admin Wilayah dilakukan oleh Super Admin (via Admin Management), bukan self-service.**
