...
const dummy = [
    ...
    { id: 2, name: 'Kab. Gowa', kantor: 'Kantor Wilayah Gowa', tipe: 'cabang', locations: [{ nama_lokasi: 'Kantor Gowa', lat: -5.3114, lng: 119.42, radius: 200 }], address: 'Jl. Poros Malino — Gowa' },
...
export default function Regions() {
    ...
    const addLoc = () => {
        if (form.locations.length >= 3) return;
    ...
    const handleSave = () => {
        ...
        if (invalid) { setToast('Lengkapi nama lokasi + koordinat'); setTimeout(() => setToast(null), 2000); return; }
        if (editing) {
            ...
            setToast('Wilayah diperbarui (frontend only)'); setOpen(false); setTimeout(() => setToast(null), 2000);
        } else {
            ...
            setToast('Wilayah ditambah (frontend only)'); setOpen(false); setTimeout(() => setToast(null), 2000);
        }
    };
            ...
            <div className="space-y-5">
                ...
                <div className="bg-white rounded-2xl shadow-[0_2px_16px_rgba(15,23,42,0.04)] overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-[#F8FAFC] text-xs font-medium text-[#64748B]">
                                <tr><th className="text-left px-4 py-3">Wilayah</th><th className="text-left px-4 py-3">Kantor</th><th className="text-left px-4 py-3">Lokasi (1–3)</th><th className="text-left px-4 py-3">Radius</th><th className="px-4 py-3"></th></tr>
                            </thead>
                                ...
                                {filtered.map((r) => (
                                    <tr key={r.id} className="hover:bg-[#F8FAFC]/50">
                                        ...
                                        <td className="px-4 py-3.5 text-xs">
                                            ...
                                            {r.locations.length > 1 && <span className="inline-block mt-1 text-xs bg-[#FFF7E6] text-[#92400E] px-2 py-0.5 rounded-full border border-[#FCB833]/20">{r.locations.length} lokasi</span>}
                                        </td>
                                        <td className="px-4 py-3.5">
                                            <div className="flex flex-col gap-1">
                                                {r.locations.map((l, i) => (<span key={i} className="inline-flex items-center gap-1 text-xs font-medium bg-[#FFF7E6] text-[#92400E] px-2.5 py-1 rounded-full border border-[#FCB833]/20 w-fit">{l.radius} m</span>))}
                                            </div>
                                        </td>
                                        <td className="px-4 py-3.5 text-right">
                                            <div className="flex gap-1 justify-end">
                                                ...
                                                <button type="button" onClick={() => handleDelete(r.id)} className="text-xs font-medium text-[#991B1B] bg-[#FEF2F2] px-3 py-1.5 rounded-lg">Hapus</button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
                    ...
                    <div className="px-4 py-3 bg-[#F8FAFC] text-xs text-[#64748B] flex items-center justify-between">
                        <span>Menampilkan {filtered.length} dari {regions.length}</span>
                        <span className="hidden lg:inline">Klik peta untuk set lat/lng • Di luar semua radius ditolak 422</span>
                    </div>
                </div>
                ...
                <div className="bg-[#FFF7E6] rounded-2xl p-4 flex gap-3">
                    ...
                    <div>
                        <p className="text-sm font-medium text-[#0F172A]">Geofence per lokasi — validasi ke jarak terdekat</p>
                        <p className="text-xs text-[#92400E]">Super Admin edit semua wilayah • Admin Wilayah edit lokasi own region saja (50–1000m)</p>
                    </div>
                </div>
                    ...
                    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={close}>
                        <div className="bg-white rounded-2xl w-full max-w-[640px] max-h-[90vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
                            <div className="sticky top-0 bg-white px-6 py-4 flex items-center justify-between border-b">
                                <h3 className="font-semibold text-[#0F172A]">{editing ? 'Edit Wilayah' : 'Tambah Wilayah'}</h3>
                            ...
                            <div className="px-6 py-5 space-y-4">
                                ...
                                <div className="pt-2">
                                    <div className="flex items-center justify-between">
                                        <h4 className="text-sm font-medium text-[#0F172A]">Lokasi Kantor (1–3)</h4>
                                        <button type="button" onClick={addLoc} disabled={form.locations.length >= 3} className="text-xs font-semibold bg-[#FFF7E6] text-[#92400E] px-3 py-1.5 rounded-lg border border-[#FCB833]/20 disabled:opacity-40">+ Tambah lokasi</button>
                                    </div>
                                    <div className="mt-3 space-y-3">
                                        {form.locations.map((l, idx) => (
                                            <div key={idx} className="bg-[#F8FAFC] rounded-xl p-3 space-y-2">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-xs font-semibold text-[#0F172A]">Lokasi {idx + 1}</span>
                                                    {form.locations.length > 1 && <button type="button" onClick={() => removeLoc(idx)} className="text-xs text-[#991B1B] font-medium">Hapus</button>}
                                                </div>
                                                <input value={l.nama_lokasi} onChange={(e) => updateLoc(idx, { nama_lokasi: e.target.value })} placeholder="Nama lokasi (ex: Kantor Utama)" className="w-full rounded-lg bg-white border border-[#E2E8F0] px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#1E3A8A]/10" />
                                                ...
                                                <p className="text-xs text-[#94A3B8]">Klik peta Leaflet untuk isi koordinat (dummy) • radius 50–1000m</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
