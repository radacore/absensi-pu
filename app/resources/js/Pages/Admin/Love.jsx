...
export default function LoveAdmin(){
    ...
    const sitesForWilayah = useMemo(()=>{
        if(isWilayah){ const r=regionsData.find((x)=>x.name===ownRegion); return r? r.locations: []; }
    ...
    const baseClaims = isWilayah ? claims.filter((c)=>c.wilayah===ownRegion) : claims;
    const filtered = useMemo(()=> baseClaims.filter((c)=>{
        ...
        if(q && !c.nama.toLowerCase().includes(q.toLowerCase()) && !String(c.kantor||'').toLowerCase().includes(q.toLowerCase())) return false;
        return true;
        ...
        <AdminLayout>
            <div className="space-y-5">
                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                    <div>
                        <h1 className="text-xl font-semibold tracking-tight text-[#0F172A]">{isWilayah ? `Klaim Toleransi — ${ownRegion}` : `Klaim Toleransi — kuota ${loveMax}/bulan`}</h1>
                ...
                <div className="bg-white rounded-2xl shadow-[0_2px_16px_rgba(15,23,42,0.04)] overflow-hidden">
                                            ...
                                            <td className="px-4 py-3 text-right">
                                                {c.status==='pending' ? (
                                                    <div className="flex gap-1 justify-end flex-wrap">
                                                        <button type="button" onClick={()=>handleApprove(c.id)} className="text-xs font-medium px-2.5 py-1.5 rounded-lg bg-[#10B981] text-white hover:bg-[#059669]">Approve</button>
                                                        {rejectNote.id===c.id ? (
                                                            <span className="flex gap-1">
                                                                ...
                                                                <button type="button" onClick={()=>handleReject(c.id)} className="text-xs font-medium bg-[#EF4444] text-white px-2 py-1.5 rounded-lg">Kirim</button>
                    ...
                    <div className="px-4 py-3 bg-[#F8FAFC] text-xs text-[#64748B]">{isWilayah ? `Admin Wilayah: review & approve hanya ${ownRegion}` : 'Super Admin lihat semua • Admin Wilayah own region saja'}</div>
                </div>
            </div>
        </AdminLayout>
    );
