            ...
            <div className="space-y-5">
                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs text-[#94A3B8] mt-1">{isWilayah ? `Scope own region ${OWN_REGION} •` : ''} Di luar radius titik assigned tidak bisa claim (422) • 1 karyawan = 1 titik • Reset 1st 00:00 WITA • Love max dari Pengaturan ({loveMax}) • cukup alasan</p>
                    </div>
                                ...
                                {filtered.map((c) => {
                                    ...
                                    return (
                                        <tr key={c.id} className="hover:bg-[#F8FAFC]/50">
                                            ...
                                            <td className="px-4 py-3 text-xs"><span title={hit ? `${c.jarak} m / ${c.radius} m • ${hit.site.nama_lokasi} • ${inRadius ? 'Dalam — bisa excuse' : 'Di luar — tidak bisa excuse'}` : ''} className={inRadius ? 'text-[#065F46]' : hit ? 'text-[#991B1B]' : 'text-[#64748B]'}>{c.jam} • {c.jarak} m{c.radius ? `/${c.radius}m` : ''} {hit ? (inRadius ? '• Dalam' : '• Di luar') : ''}</span><p className="text-xs text-[#94A3B8] truncate max-w-[200px]" title={c.alasan}>{c.alasan}</p></td>
