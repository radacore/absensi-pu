...
const initialLate = [
    { id: 101, tgl: '24 Agu 2026', jam: '07:52', jarak: 42, status: 'late' },
];
...
export default function Love() {
    ...
    const handleClaim = () => {
        ...
        if (sisa <= 0) return;
        if (!assigned) return;
        // late valid only if within assigned radius — mock check
        if (selected.jarak > assigned.site.radius) return;
        const newClaim = {
            id: Date.now(),
        ...
        <KaryawanLayout>
            <div className="space-y-5">
                <div>
                    ...
                    <p className="text-sm text-[#64748B]">4 Love/bulan • Reset 1 Sep 00:00 WITA {assigned ? `• ${assigned.site.nama_lokasi} • ${assigned.site.radius} m` : '• Tanpa titik'}</p>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(15,23,42,0.04)]">
                    ...
                    <p className="text-xs text-[#94A3B8] mt-3">Pakai 1 Love untuk 1 keterlambatan <span className="font-medium text-[#0F172A]">dalam radius {assigned ? `${assigned.site.nama_lokasi} ${assigned.site.radius} m` : 'titik assigned'}</span> + dokumen (bulan yang sama — hari beda boleh) → approval 1 level {assigned ? `Admin ${assigned.region.name}` : 'Admin Wilayah'}</p>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-[0_2px_16px_rgba(15,23,42,0.04)]">
                    ...
                    <p className="text-xs text-[#94A3B8] mt-1">Hanya late bulan yang sama (hari beda boleh) & dalam radius {assigned ? `${assigned.site.nama_lokasi} ${assigned.site.radius} m` : 'titik assigned'} • Di luar radius ditolak 422</p>
