                ...
                {navItems.map((it) => {
                    ...
                    return (
                        <Link
                            key={it.href}
                            href={it.href}
                            className={`flex flex-col items-center justify-center min-w-[56px] min-h-[44px] rounded-xl px-2 py-1 transition ${active ? 'text-[#1E3A8A]' : 'text-[#6B7280]'}`}
                        >
                            <span className={`p-1.5 rounded-lg ${active ? 'bg-[#E0F2FE]' : 'bg-transparent'}`}>{it.icon(active)}</span>
                            <span className={`text-[10px] mt-1 ${active ? 'font-semibold text-[#1E3A8A]' : 'font-medium'}`}>{it.label}</span>
                        </Link>
                    );
                })}
            </nav>
